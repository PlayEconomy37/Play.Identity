...
var (
	// ErrInvalidHash is the error returned by ComparePasswordAndHash() if the provided
	// hash isn't in the expected format.
	ErrInvalidHash = errors.New("argon2id: hash is not in the correct format")

	// ErrIncompatibleVariant is the error returned by ComparePasswordAndHash() if the
	...
	// ErrIncompatibleVersion is the error returned by ComparePasswordAndHash() if the
...
// the Argon2 reference C implementation. It looks like this:
//
...
// generateRandomBytes generates a cryptographically secure random salt
